...
dataset_info = dataset_params['bindingmoad']  # NOTE: for finetuning, we use the Binding MOAD dataset's metadata
...
if __name__ == '__main__':
    ...
    pdbdir = args.basedir / 'finetuning_set/'

    # Make output directory
    if args.outdir is None:
        suffix = '' if 'H' in atom_dict else '_noH'
        suffix += '_ca_only' if args.ca_only else '_full'
        processed_dir = Path(args.basedir, f'finetuning_set_processed{suffix}')
    else:
    ...
    data_split = {}
    splits_to_process = ['test', 'val', 'train']
    for split in splits_to_process:
        pocket_ids = pdbdir.glob("*.pdb")
        # (id, protein, molecule tuple)
        data_split[split] = [(None, x.stem.split('_')[0][:4], (x.stem.split('_')[1],))
                        for x in pocket_ids]
    ...
    for split in data_split.keys():
        ...
        with tqdm(total=n_tot) as pbar:
            for p in pair_dict:
                ...
                # try all available .pdb files
                for pdbfile in sorted(pdbdir.glob(f"{p.lower()}*.pdb")):
                    ...
                    if split in {'val', 'test'} and n_pdb_successful > 0:
